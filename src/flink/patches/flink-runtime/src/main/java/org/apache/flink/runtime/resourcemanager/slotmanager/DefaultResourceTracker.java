...
public class DefaultResourceTracker implements ResourceTracker {
    ...
    public DefaultResourceTracker(boolean isCustomScheduler) {
    ...
    public void notifyResourceRequirements(
        ...
        JobScopedResourceTracker tracker = getOrCreateTracker(jobId);
        if (isCustomScheduler) {
            LOG.info(
                    "[KUBEFLINK] DefaultResourceTracker.notifyResourceRequirements(custom) BEFORE job={} requirements={} acquired={} missing={}",
                    jobId,
                    tracker.getCurrentRequirementsForDebug(),
                    tracker.getAcquiredResources(),
                    tracker.getMissingResources());
        } else {
            LOG.trace(
                    "[KUBEFLINK] DefaultResourceTracker.notifyResourceRequirements(normal) job={} newRequirements={}",
                    jobId,
                    resourceRequirements);
        }

        tracker.notifyResourceRequirements(resourceRequirements, isCustomScheduler);

        if (isCustomScheduler) {
            // LOG after
            LOG.info(
                    "[KUBEFLINK] DefaultResourceTracker.notifyResourceRequirements(custom) AFTER job={} requirements={} acquired={} missing={}",
                    jobId,
                    tracker.getCurrentRequirementsForDebug(),
                    tracker.getAcquiredResources(),
                    tracker.getMissingResources());
    ...
    private JobScopedResourceTracker getOrCreateTracker(JobID jobId) {
        return trackers.computeIfAbsent(
                ...
                ignored -> {
                    ...
                    return new JobScopedResourceTracker(jobId, isCustomScheduler);
                });
    }
    ...
    public Map<JobID, Collection<ResourceRequirement>> getMissingResources() {
        ...
        if (!allMissingResources.isEmpty()) {
            LOG.info("[KUBEFLINK] DefaultResourceTracker.getMissingResources missing={}", allMissingResources);
        }
