...
public class ResourceProfile implements Serializable {
    ...
    public boolean isMatching(final ResourceProfile required) {
        ...
        if (required.equals(UNKNOWN)) {
            // If the required profile carries a preferredLocation, this profile must match it.
            // Without this check, concreteProfile_TM3.isMatching(UNKNOWN_TM1) would return true
            // because UNKNOWN_TM1.equals(UNKNOWN) is true (null vs non-null location falls through
            // to field comparison). That causes the requirement matcher to credit allocations to
            // the wrong TM requirement.
            if (required.preferredLocation != null) {
                if (this.preferredLocation == null
                        || !this.preferredLocation.contains(required.preferredLocation)) {
                    return false;
                }
            }
            return true;
